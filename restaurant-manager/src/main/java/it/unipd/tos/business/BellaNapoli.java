...
public class BellaNapoli implements RestaurantBill{
	...
	public double getOrderPrice(List<MenuItem> itemsOrdered) throws RestaurantBillException {
		...
		double tot = itemsOrdered.stream().mapToDouble(x -> x.getPrice()).sum();
		
		if(getNumPizza(itemsOrdered) > 10)
			tot -= itemsOrdered.stream().mapToDouble(x -> x.getPrice()).min().orElse(0);
	...
	public int getNumPizza(List<MenuItem> itemsOrdered) {
		return (int) itemsOrdered.stream().filter(x -> typeItem.PIZZA.equals(x.getItemType())).count();
	}
}
